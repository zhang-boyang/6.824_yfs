...
rsm::recovery()
{
  ...
  while (1) {
    ...
    if (vid_insync != vid_commit){
        continue;
        tprintf("vid_insync:%d!=vid_commit:%d", vid_insync, vid_commit);
    }
...
rsm::invoke(int proc, viewstamp vs, std::string req, int &dummy)
{
  ...
  if (inviewchange || primary == cfg->myaddr() || vs != myvs){
      /*
        tprintf("inviewchange:%d, primary == cfg->myaddr():%d, vs != myvs:%d\n",
                inviewchange, primary == cfg->myaddr(), vs != myvs);
        tprintf("vs.seqno:%u, vs.vid:%u, myvs.seqno:%u, myvs.vid:%u\n", vs.seqno, vs.vid, myvs.seqno, myvs.vid);
      */
...
rsm_protocol::transferres &r)
{
  ...
  /*
  {
    ...
    Base64 base;
    std::string r_r =  r.state;
    r_r = base.Encode(r_r.c_str(), r_r.size());
    tprintf("base64:mashall :r_r:%s, r_r size:%zu\n", r_r.c_str(), r_r.size());
    stf->unmarshal_state(base.Decode(r_r.c_str(), r_r.size()));
    r_r = stf->marshal_state();
    r_r = base.Encode(r.state.c_str(), r.state.size());
    tprintf("base64:after unmashall :r_r:%s, r_r size:%zu\n", r_r.c_str(), r_r.size());
  }
  */
  r.last = last_myvs;
  return ret;
