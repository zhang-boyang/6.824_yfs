 ...
 *
 *      Created by zhangby;
 *      @author liruixing(search the code on google)
 */
...
private:
     ...
     */
    std::string Encode(const char * str,std::size_t bytes);
    std::string Decode(const char *str,std::size_t bytes);
};

std::string Base64::Encode(const char * str,std::size_t bytes) {
    int num = 0,bin = 0,i;
...
std::string Base64::Decode(const char *str,std::size_t length) {
       //解码表
