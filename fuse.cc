                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    yfs_client::status ret = yfs->setattr(ino, attr);
    if (ret != yfs_client::OK) {
                ...
                off_t off, struct fuse_file_info *fi)
{
  ...
  yfs_client::status ret = yfs->read(ino, size, off, buf);
  if (ret != yfs_client::OK){
	  fuse_reply_err(req, ENOENT);
	  return;
  }
  //printf("fuseserver_read off is %d size is %d ino is %llu buf is %s, data is %s, datasize %u", off, size, ino, buf.c_str(), data.c_str(), data.size());
                 ...
                 struct fuse_file_info *fi)
{
  ...
  // Change the above line to "#if 1", and your code goes here
	...
	yfs_client::status ret = yfs->write(ino, size, off, buf);
	if (ret != yfs_client::OK) {
		fuse_reply_err(req, ENOENT);
		return;
	}
  //printf("fuseserver_write ino is %lld buf is %s, off is %d size is %u", ino, write_data.c_str(), off, size);
                  ...
                  mode_t mode, struct fuse_file_info *fi)
{
  ...
  printf("fuseserver_create: ret=%d\n", ret);
}
