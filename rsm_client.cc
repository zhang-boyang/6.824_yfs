...
rsm_client::invoke(int proc, std::string req, std::string &rep)
{
  ...
  ScopedLock ml(&rsm_client_mutex);
  while (1) {
    tprintf("rsm_client::invoke proc %x primary %s\n", proc, primary.c_str());
    handle h(primary);
    ...
    tprintf("rsm_client::invoke proc %x primary %s ret %d\n", proc,
           primary.c_str(), ret);
...
prim_fail:
    ...
    primary_failure();
    tprintf ("rsm_client::invoke: retry new primary %s\n", primary.c_str());
  }
  return ret;
}
...
{
  tprintf("rsm_client::init_members get members!\n");
  handle h(primary);
  ...
  if (cl == 0 || ret != rsm_protocol::OK)
    return false;
  if (new_view.size() < 1) {
    tprintf("rsm_client::init_members do not know any members!\n");
    VERIFY(0);
  }
  known_mems = new_view;
  primary = known_mems.back();
  ...
  tprintf("rsm_client::init_members: primary %s\n", primary.c_str());
